  ...
  return (
    <Box>
      ...
      >
        ...
        <Divider orientation='vertical' sx={{bgcolor:'#000', height: '52px', width: '1.2px', mt:'1.2px', borderRadius: '30px', mr:'4px', ml:'3px'}}/>

        <Box display='flex' flexDirection='column'>
        <FormGroup>
          <FormControlLabel
              control={<Checkbox onClick={handleClick} disabled={isChecked} sx={{ color: 'grey', p:1.2, py:0, '& .MuiSvgIcon-root': { fontSize: '1rem' } }} />}
              label={<Box sx={{color:approveColor()}}>Approve</Box>}
              sx={{ mb: 0 }}
              
          />
          <FormControlLabel
              control={<Checkbox onClick={handleClick} disabled={isChecked} sx={{ color: 'grey', p:1.2, '& .MuiSvgIcon-root': { fontSize: '1rem' } }} />}
