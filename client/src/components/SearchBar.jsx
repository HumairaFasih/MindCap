...
function SearchBar({ onData }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      instance
        .post(`search/${searchType}`, searchData)
        .then((result) => {
      ...
      >
        <Grid item xs={10} md={6} lg={6}>
          <TextField
            fullWidth
            label={`Search ${
              usertype === 'Student' ? 'Counselor' : 'Student/Counselor'
            }`}
            value={query}
        ...
        >
          <Grid item xs={6} md={2.4} lg={2.4}>
            <Filter
              ...
              menuItems={menuItemsAccountType}
            />
          </Grid>
          <Grid item>
